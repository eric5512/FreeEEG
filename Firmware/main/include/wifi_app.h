...
// UDP communication
#define UDP_DATA_BUFF_SIZE 10

typedef struct {
  uint8_t num_elec;
  uint32_t elec_data[UDP_DATA_BUFF_SIZE][8];

  uint8_t num_imu;
  uint16_t accel_data[UDP_DATA_BUFF_SIZE][3];
  uint16_t gyro_data[UDP_DATA_BUFF_SIZE][3];

  uint8_t num_events;
  uint8_t events[UDP_DATA_BUFF_SIZE];
} udp_packet_t;

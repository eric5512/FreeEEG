...
// UDP communication
#define UDP_ELEC_BUFF_SIZE  30
#define UDP_IMU_BUFF_SIZE   5
#define UDP_EVENT_BUFF_SIZE 2

typedef struct  __attribute__((packed)) {
  uint32_t abs_time;
  uint8_t num_elec;
  uint32_t elec_time[UDP_ELEC_BUFF_SIZE];
  uint32_t elec_data[UDP_ELEC_BUFF_SIZE][8];

  uint8_t num_imu;
  uint32_t imu_time[UDP_IMU_BUFF_SIZE];
  uint16_t accel_data[UDP_IMU_BUFF_SIZE][3];
  uint16_t gyro_data[UDP_IMU_BUFF_SIZE][3];

  uint8_t num_events;
  uint32_t events_time[UDP_EVENT_BUFF_SIZE];

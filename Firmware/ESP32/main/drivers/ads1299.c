...
static uint8_t ads1299_sample_rate_bits(uint16_t sample_rate) {
    switch (sample_rate) {
        ...
        default:    return 0x06;   // Safe default: 250 SPS
    }
}
...
void ads1299_init(uint8_t pin_mosi, uint8_t pin_miso, uint8_t pin_sck, uint8_t pin_cs) {
    ...
    ESP_ERROR_CHECK(spi_bus_initialize(ADS1299_SPI_HOST, &buscfg, SPI_DMA_CH_AUTO));
    ESP_ERROR_CHECK(spi_bus_add_device(ADS1299_SPI_HOST, &devcfg, &ads_spi));
    ...
    if ((id_reg & 0b00011100) == 0b00011100) {
      char n_channels = ((id_reg & 0b11) == 0b10) ? '8' : (((id_reg & 0x11) == 0b01) ? '6' : '4');
      ESP_LOGI(TAG, "Found ADS1299 with %c channels", n_channels);
    } else {
      ESP_LOGE(TAG, "ADS1299 not found");
    }
}
...
void ads1299_read(RingbufHandle_t* ads1299_ringbuff) {
    if (ads1299_ringbuff == NULL || *ads1299_ringbuff == NULL) {
        return;
    }

    uint8_t tx[ADS1299_FRAME_SIZE] = {0};
    uint8_t ads_frame_buff[ADS1299_FRAME_SIZE];


    memset(&ads_frame_buff, 0, sizeof(ads_frame_buff));

    ads1299_spi_txrx(tx, ads_frame_buff, ADS1299_FRAME_SIZE);

    xRingbufferSend(
		    *ads1299_ringbuff,
		    &ads_frame_buff,
		    sizeof(ads_frame_buff),
		    pdMS_TO_TICKS(10)
		    );
}
